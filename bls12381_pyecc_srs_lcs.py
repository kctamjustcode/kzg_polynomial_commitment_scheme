...
def ecp_sum(pl: list):
    pt_sum = bls12381_Z1
    for pt in pl:
        if bls12381_is_on_curve(pt, bls12381_b):
            pt_sum = bls12381_add(pt_sum, pt)
        else:
            pt_sum = bls12381_add(pt_sum, bls12381_Z1)
    return pt_sum

def ecp_sum_twist(pl: list):
    pt_sum = bls12381_Z2
    for pt in pl:
        if bls12381_is_on_curve(pt, bls12381_b2):
            pt_sum = bls12381_add(pt_sum, pt)
        else:
            pt_sum = bls12381_add(pt_sum, bls12381_Z2)
    return pt_sum
...
def commit_srs(rl: list):
    ...
    return ecp_sum([bls12381_multiply(srs_1[i], lag_coef[i] ) for i in range(l)]), lag_coef
...
gfp_12_one = bls12381_pairing(bls12381_G2, bls12381_G1)/bls12381_pairing(bls12381_G2, bls12381_G1)
...
def batchverify_s3(c_list, abp, zinvbp_list, rxp_list):
    lhs_F = gfp_12_one
    for i in range(nc):
        lhs_F *= bls12381_pairing(zinvbp_list[i], bls12381_multiply(bls12381_add(c_list[i], bls12381_multiply(rxp_list[i], -1%dp) ) , gam_pw[i]))
    rhs = bls12381_pairing(zt_p, abp)
    return lhs_F == rhs
...
def batchprove_s4(rmplist: list, rxplist: list, rzlist: list, zbiplist: list, zbizlist: list):
    ...
    W = ecp_sum([bls12381_multiply( srs_1[i], h_x[i] ) for i in range(len(h_x))])
    F1 = ecp_sum([bls12381_multiply( srs_1[i], fz_x[i] ) for i in range(len(fz_x))])
    Wp = ecp_sum([bls12381_multiply( srs_1[i], wp_x[i] ) for i in range(len(wp_x))])
    F = bls12381_add(F1, bls12381_multiply(W, -z_tz %dp))
    return F, W, Wp #return W, Wp

def batchverify_s4(fin, wpin):
    return bls12381_pairing(srs_2[0], fin) == bls12381_pairing(bls12381_add(srs_2[1], bls12381_multiply(srs_2[0], -r_z %dp)), wpin)

# sect4.1: finalized one
def batchverify_s41(fin, wpin):
    fn = bls12381_add(fin, bls12381_multiply(wpin, r_z))
    return bls12381_pairing(srs_2[0], fn) == bls12381_pairing(srs_2[1], wpin)
...
rxtp_list = [ ecp_sum([bls12381_multiply(srs_1[j], rxcf_list[i][j]) for j in range(len(rxcf_list[i]))]) for i in range(nc) ]

ztcf = z_poly_modp([i for i in range(l)], dp)
zt_p = ecp_sum_twist([bls12381_multiply(srs_2[i], ztcf[i]) for i in range(len(ztcf))])

zinvb_cflist = [ z_inv_modp(rib_list[i], l, dp) for i in range(nc) ]
zinvb_plist = [ ecp_sum_twist([ bls12381_multiply(srs_2[j], zinvb_cflist[i][j]) for j in range(len(zinvb_cflist[i])) ]) for i in range(nc) ]
...
if bvs3:
    print('bls12381 version aggre-proof sect3 batch-opening verified: ', t01-t0)
...
if batchverify_s4(F, Wp):
    print('bls12381 version aggre-proof sect4 batch opening verified')
if batchverify_s41(F, Wp):
    print('bls12381 version aggre-proof sect4.1 batch opening verified')
