...
def prove_srs(rl: list, ind: int, da: int):
    ...
    return ecp_sum([bn128_multiply(srs_1[i], quot_coef_mp[i]) for i in range(len(quot_coef))])
...
### batch-opening
def Z(id_list: list):
    return sympy.expand(sympy.prod([x - id_list[i] for i in range(len(id_list))]))
...
def mbatchprove(rl: list, dx: list, dy: list):
    ...
    pf_point = ecp_sum([bn128_multiply(srs_1[i], qb_coef_mp[i]) for i in range(len(qb_coef_mp))])
    return pf_point
...
def mbatchverifyeval(C, P, dx: list, dy: list):
    ...
    zx_coef_mp = [(zx_coef[i])%d_p for i in range(len(zx_coef))]

    lhs_lfpt = ecp_sum2( [bn128_multiply(srs_2[i], zx_coef_mp[i]) for i in range(len(zx_coef_mp))] )
...
#srs_1 = [bn128_multiply(bn128_G1, rfp_list[0])]
#srs_2 = [bn128_multiply(bn128_G2, rfp_list[1])]
srs_1 = [bn128_G1]
srs_2 = [bn128_G2]
for i in range(1, l):
    srs_1.append(bn128_multiply(srs_1[-1], r_pub))
...
for _ in range(10):
    ...
    print('batchprove time: ', ttt2-tvn)
    print('batchverify time: ', tt2-ttt2)
    '''
...
def batchverifyeval_arb(rs: int, rfp: int, C, P, dx: list, dy: list):
    ...
    rhs_bnap = bn128_pairing(bn128_G2, bn128_add(C, bn128_multiply(bn128_G1, (-dlitpe*rfp)%d_p)))
    return lhs_bnap == rhs_bnap

def batchverifyeval_ped_arb(rs: int, rfpl: list, AC, AP, dx: list, y: list):
    assert len(rfpl) == len(y) != 1
    aggre_vec = ecp_sum( [ bn128_multiply(bn128_G1, -interpolate_eval_fast(dx, y[i], rs, d_p)*rfpl[i]%d_p)  for i in range(len(rfpl))] )
    z_e = poly_eval_modp(z_poly_modp(dx, d_p), rs, d_p)
    lhs_bnap = bn128_pairing(bn128_multiply(bn128_G2, z_e), AP)
    rhs_bnap = bn128_pairing(bn128_G2, bn128_add(AC, aggre_vec))
    return lhs_bnap==rhs_bnap
...
if batchverifyeval_ped_arb(r_pub, rfp_list, AGC, AGBP, rib_list, rdb_ped_list):
    print('BN128 Pedersen auto_batch opening verified, arbitrary points and arbitrary number of commitments')
