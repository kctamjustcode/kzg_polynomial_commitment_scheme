...
def batchverifyeval_ped_srs(AC, AP, dx: list, rdb: list):   # rfp_list is 'missing'
    ...
    aggre_vec = ecp_sum([ecp_sum([bn128_multiply(srs_1[i], (-lag_list[j][i])%d_p) for i in range(len(lag_list[j]))]) for j in range(nc)])
    zx_cofe = z_poly_modp(dx, d_p)

    lhs_lfpt = ecp_sum_twist([bn128_multiply(srs_2[i], zx_cofe[i]) for i in range(len(zx_cofe))])
    lhs_bnap = bn128_pairing(lhs_lfpt, AP)
    rhs_bnap = bn128_pairing(srs_2[0], bn128_add(AC, aggre_vec))
    return lhs_bnap==rhs_bnap
